...
import { upsertIssue, upsertPR, getSyncState, setSyncState, saveDb, } from "../memory/db.js";
...
async function getChangedAreas(prNumber) {
    try {
        ...
        const areas = new Set(files.map(f => f.filename.split("/").slice(0, 3).join("/")));
...
async function getKeyComment(prNumber) {
    try {
        ...
        const meaningful = comments.find(c => !c.user.login.includes("[bot]") &&
...
export async function syncDelta() {
    ...
    const newlyMerged = recentPRs.filter(pr => pr.merged_at && pr.merged_at >= lookback);
