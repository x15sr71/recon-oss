...
function loadRaw() {
  ...
  try {
    ...
    if (!Array.isArray(parsed.exchanges)) parsed.exchanges = [];
    if (!parsed.window_size) parsed.window_size = CONFIG.limits.chatHistory;
    return parsed;
  } catch {
    return { window_size: CONFIG.limits.chatHistory, exchanges: [] };
  }
...
export function appendReplyToLastExchange(replyText) {
  const history = loadRaw();
  if (!history.exchanges.length) return;
